...
static const uint32_t INA226_I2C_TIMEOUT = 1000;
...
INA226::INA226(I2CMaster &i2c, uint16_t addr) : m_i2c(i2c), m_addr(addr << 1)
{
...
{
	return m_i2c.deviceReady(m_addr);
}
...
uint16_t INA226::_writeRegister(uint8_t reg, uint16_t value)
{
	...
	if (!m_i2c.write(m_addr, data,  3,  INA226_I2C_TIMEOUT))
	{
		_error = -1;
		return 0;
	}
...
{
	uint8_t data[2];
	if (!m_i2c.write(m_addr, &reg, 1, INA226_I2C_TIMEOUT))
	{
		_error = -1;
		return 0;
	}

	if (!m_i2c.read(m_addr, data, 2, INA226_I2C_TIMEOUT))
	{
		_error = -2;
		return 0;
	}
